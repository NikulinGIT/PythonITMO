...
#функция проставления оценок магазину
def mod_SQL(dbname_name,user_name,password_name,host_name):
    while True:
      try:
        Question1=input('Вы хотите оценить компанию? (yes/no) ')
        if Question1=='yes':
          Name_str = input('Введите ID магазина ')
          Stars=int(input('оцените магазин по шкале от 1 до 5 '))
          if (Stars)>5 or (Stars)<1:
            print("Оценка не действительна")
          else:
            conn_SQL = psycopg2.connect(dbname=dbname_name, user=user_name, password=password_name, host=host_name,port="5432")
            cursor = conn_SQL.cursor()
            #выставление оценки магазину
            cursor.execute("UPDATE Markets_info SET Stars= (%s) WHERE FMID = (%s);", (Stars,Name_str))
            print("Спасибо за Вашу оценку")
            #размещение отзыва о магазине
            Opinions=input('Вы можете здесь оставить отзыв ')
            Name_opin = os.getlogin()
            Opinions=Name_opin+':'+ Opinions
            cursor.execute("UPDATE Markets_info SET Opinions= (%s) WHERE FMID =(%s);", (Opinions,Name_str))
        elif Question1=='no': break
      except:
        print("Текст не введен")

#функция удаления поставленных оценок магазину
def del_SQL(dbname_name,user_name,password_name,host_name):
    conn_SQL =  psycopg2.connect(dbname=dbname_name, user=user_name, password=password_name, host=host_name, port="5432")
    cursor = conn_SQL.cursor()
    while True:
      try:
        Question1=input('Вы хотите удалить оценку? (yes/no) ')
        if Question1=='yes':
          Name_str = int(input('Введите ID магазина '))
          cursor.execute('UPDATE Markets_info SET Stars =%s, Opinions = %s WHERE FMID = %s;', ('','',Name_str))
          print("Оценка удалена")
        elif Question1 == 'no': break
      except:
          print("Ошибка ввода текста")
  ...
  try:
    ...
    while True:
      try:
        ...
        del_SQL(dbname_name, User_name, Password_name, Host_name)
        Question1=input('Вы хотите закончить работу? (yes/no) ')
        if Question1=='yes': break
      except:
        print("Ошибка ввода текста")